    ...
    <#if kierros??>
  	<table>
   		<tr>
	   		...
	   		<tr>
    			<td>Valitse kenttä:</td><td>${kierros.kentta_id?c}</td>
    			# Kenttiä varten tehtävä valintalista, josta haetaan
    		...
    		<tr>
    			<td>Tii, jolta pelattu:</td><td>${kierros.tii_ID}</td>
    			<#list tiit as tii>
    				<#if tii.kentta_id == kentta.kentta_id>${tii.kentan_nimi}</#if>
    			</#list>
